...
class BoardController: NSObject,
  ...
  private func applyNumLettersSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
          // 2. Check if the key exists in the settings dictionary and if its value is an integer.
          if let numLetters = settings[kNumLettersKey] as? Int {
              // 3. Assign the value to the numItemsPerRow property.
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
          ...
          // 2. Check if the key exists in the settings dictionary and if its value is a String.
          if let themeValue = settings[kWordThemeKey] as? String {
              // 3. Use the WordTheme(rawValue:) initializer to convert the theme string to a WordTheme enum.
              if let theme = WordTheme(rawValue: themeValue) {
                  // 4. Generate a new goal word using the theme and assign it to the goalWord property.
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let isAlienWordleValue = settings[kIsAlienWordleKey] as? Bool {
              // 3. Assign the boolean value to the isAlienWordle property.
